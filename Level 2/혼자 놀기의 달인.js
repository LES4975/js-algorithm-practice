function solution(cards) {
  ...
  let group1 = 0,
    group2 = 0;
  function openBox(box, index, group) {
    if (box[index] === 0) return group;
    else {
      group++;
      tmp = cards[index];
      box[index] = 0;
      index = tmp - 1;
      return openBox(box, index, group);
    }
  }
  cards.forEach((card) => {
    let arr = [...cards];
    let visited = new Array(arr.length).fill(1);
    const score1 = openBox(visited, card - 1, group1);
    arr.forEach((card2) => {
      if (visited[card2 - 1] !== 0) {
        const score2 = openBox(visited, card2 - 1, group2);

function solution(strs, t) {
  ...
  function compareString(str, prev) {
    if (str === t) return prev;
    else {
      let result = t.length,
        i;
      let tmp = t.substr(str.length);
      for (i = 0; i < last_arr.length; i++) {
        if (tmp.startsWith(last_arr[i])) {
          let new_str = str + last_arr[i];
          if (t.startsWith(new_str)) {
            result = Math.min(compareString(new_str, prev + 1), result);
          }
        }
      }
  ...
  for (let i = 0; i < strs.length; i++) {
    t.startsWith(strs[i]) ? start_arr.push(strs[i]) : last_arr.push(strs[i]);
  }

  start_arr.forEach((str) => {
    let count = compareString(str, 1);
    if (count > 0) answer = Math.min(answer, count);
  });
  if (answer === t.length) answer = -1;
  console.log(answer);
  return answer;
}
